...
HTTP **`/api/event`**, event deduplication, **watch-all** (`gitRepoOwners: []`), and **public GRASP retention** (bare only if `clone[]` has this host): [gittr-enhancements.md](gittr-enhancements.md), [GIT_NOSTR_BRIDGE_SETUP.md](GIT_NOSTR_BRIDGE_SETUP.md).
